...
public final class EventPublisherStepFactory implements StepFactory {
    ...
    public Step create(StepDefinition definition) {
        ...
        String event = config.requiredString("event");
        String publisherBeanName = config.optionalString("publisher");
        return new EventPublisherStep(name, event, resolvePublisher(publisherBeanName), evaluator);
    }
